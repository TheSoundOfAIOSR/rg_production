...
async def do_things():
    ...
    df = pd.read_csv('Internal pipeline evaluation - TTS Only.csv')
    print(df)
    input_text = df['Text Input'].to_list()
    print(input_text)
    qualities = []

    for i in input_text:
        resp = await tts.process_text(i)
        qualities.append(resp['resp']['qualities'])

    with open('output.csv', 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerows(zip(input_text, qualities))
